...
my %wunderlist_transtable_EN = ( 
  ...
  "gotodetail"        =>  "Click to show detail page of wunderlist device",
  "newentry"          =>  "Add new task to list",
  "nolistdata"        =>  "No data for this list",
);

my %wunderlist_transtable_DE = ( 
  "check"             =>  "Erledigen",
  "delete"            =>  "Löschen",
  ...
  "gotodetail"        =>  "Detailseite des wunderlist-Devices aufrufen",
...
sub wunderlist_Initialize($) {
  ...
  $hash->{FW_detailFn}  = "wunderlist_detailFn";
...
sub wunderlist_CreateTask($$) {
  ...
  if ($check==1) {
    ...
    map {FW_directNotify("#FHEMWEB:$_", "if (typeof wunderlist_ErrorDialog === \"function\") wunderlist_ErrorDialog('$name','$title ".$wunderlist_tt->{"alreadythere"}."','".$wunderlist_tt->{"error"}."')", "")} devspec2array("TYPE=FHEMWEB");
...
sub wunderlist_HandleTaskCallback($$$){
  ...
  if($data =~ /created_at/ || $data =~ /completed/ || $data eq "") {
    ...
    if ($param->{wType} =~ /(complete|delete)/) {
      ...
      map {FW_directNotify("#FHEMWEB:$_", "if (typeof wunderlist_removeLine === \"function\") wunderlist_removeLine('$name','$taskId')", "")} devspec2array("TYPE=FHEMWEB");
    }
    if ($param->{wType} eq "create") {
      map {FW_directNotify("#FHEMWEB:$_", "if (typeof wunderlist_addLine === \"function\") wunderlist_addLine('$name','$taskId','$title')", "")} devspec2array("TYPE=FHEMWEB");
    }
  }
...
sub wunderlist_GetTasks($;$) {
  ...
  map {FW_directNotify("#FHEMWEB:$_", "if (typeof wunderlist_addLoading === \"function\") wunderlist_addLoading('$name')", "")} devspec2array("TYPE=FHEMWEB");
...
sub wunderlist_clearList($) {
  ...
  if ($i==0) {
    map {FW_directNotify("#FHEMWEB:$_", "if (typeof wunderlist_removeLoading === \"function\") wunderlist_removeLoading('$name')", "")} devspec2array("TYPE=FHEMWEB");
  }
}
...
# show widget in detail view of wunderlist device
sub wunderlist_detailFn(){
  ...
  return wunderlist_Html($name,undef,1);
}
...
sub wunderlist_Html(;$$$) {
  ...
  if (!$refreshGet) {
    ...
    $rot .= " <script type=\"text/javascript\">
                wunderlist_tt={};
                wunderlist_tt.refreshList='".$wunderlist_tt->{'refreshList'}."';
                wunderlist_tt.clearList='".$wunderlist_tt->{'clearList'}."';
                wunderlist_tt.clearconfirm='".$wunderlist_tt->{'clearconfirm'}."';
                wunderlist_tt.delconfirm='".$wunderlist_tt->{'delconfirm'}."';
                wunderlist_tt.check='".$wunderlist_tt->{'check'}."';
                wunderlist_tt.delete='".$wunderlist_tt->{'delete'}."';
              </script>";
    # Javascript and CSS
    $rot .= " <script type=\"text/javascript\" src=\"$FW_ME/www/pgm2/wunderlist.js?version=".$version."\"></script>
                <style id=\"wunderlist_style\">
    ...
    if (!IsDisabled($name) && ($showList || $detail)) {
      ...
      if (!$refreshGet) {
        ...
        $ret .= "<tr class=\"devTypeTr\">\n".
                " <td colspan=\"3\">\n".
                "   <div class=\"wunderlist_devType wunderlist_devType_".$name." col_header\">\n".
                    (!$FW_hiddenroom{detail}?"<a title=\"".$wunderlist_tt->{'gotodetail'}."\" href=\"/fhem?detail=".$name."\">":"").
                      AttrVal($name,"alias",$name).
                    (!$FW_hiddenroom{detail}?"</a>":"").
                "   </div>\n".
                " </td>\n".
                "</tr>\n";
        $ret .= "<tr><td colspan=\"3\"><table class=\"block wide sortable\" id=\"wunderlistTable_".$name."\">\n"; 
      ...
      foreach (@{$hash->{helper}{TIDS}}) {
        ...
        $ret .= "<td class=\"col2 wunderlist_delete\">\n".
                " <a title=\"".$wunderlist_tt->{'delete'}."\" href=\"#\" class=\"wunderlist_delete_".$name."\" data-id=\"".$_."\">\n".
                "   x\n".
                " </a>\n".
                "</td>\n";
      ...
      if (!$refreshGet) {
        ...
        $ret .= "<tr class=\"sortit odd wunderlist_ph\"".($showPH!=1?" style=\"display:none;\"":"").">";
        $ret .= " <td colspan=\"".$cs."\">".
                "   ".$wunderlist_tt->{'nolistdata'}.".\n".
                " </td>";
        $ret .= "</tr>";
        ...
        $ret .= "<td colspan=\"".$cs."\">".
                ...
                " <input title=\"".$wunderlist_tt->{'newentry'}."\" type=\"text\" id=\"newEntry_".$name."\" />\n".
                "</td>";
...
sub wunderlist_ReloadTable($) {
  my ($name) = @_;
  
  my $ret = wunderlist_Html($name,1);
  $ret =~ s/\"/\'/g;
  $ret =~ s/\n//g;
  
  map {FW_directNotify("#FHEMWEB:$_", "if (typeof wunderlist_reloadTable === \"function\") wunderlist_reloadTable('$name',\"$ret\")", "")} devspec2array("TYPE=FHEMWEB");
}
